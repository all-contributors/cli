...
test('should be able to specify custom badge template', () => {
  ...
  expect(formatBadge(options, _.times(_.constant({}), 8))).toBe('We have 8 contributors');
  expect(formatBadge(options, _.times(_.constant({}), 16))).toBe('We have 16 contributors');
});
